...
#         if len(vec) != len(input_vector): 
#             raise ValueError("Vectors must have the same dimensionality")
#         distance = sum(np.abs(vec[i] - input_vector[i])**p for i in range(len(input_vector)))**(1/p)
#         distance_vector = np.append(distance_vector, distance)
#     return distance_vector
...
#         if len(vec) != len(input_vector):
#             raise ValueError("Vectors must have the same dimensionality")
#         distance = sum((vec[i] - input_vector[i])**2 for i in range(len(input_vector)))
#         distance_vector = np.append(distance_vector, distance)
#     return distance_vector
...
def plot_efficient_3d_data(data, point_size=10):
    ...
    ax.set_yticks(np.linspace(0, 0.5, num=3))
    ax.set_zticks(np.linspace(0, 0.5, num=3))

    # Set the same background color for all panels
...
plot_efficient_3d_data(modified_sim)
