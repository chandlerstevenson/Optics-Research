...
def compute_eigenvals_rank2(snr_val):
    ...
    phi_3 = np.pi * np.random.rand(10000) 
    phi_4 = np.pi * np.random.rand(10000)

    E1 = np.random.rand(10000)
    E2 = np.random.rand(10000)
    E3 = np.random.rand(10000)
    E4 = np.random.rand(10000)
    ...
    # Check over this !!!!!!!!
    # G = np.array([[np.mean(y * np.conj(y)), np.mean(y * np.conj(v)), np.mean(y * np.conj(z)), np.mean(y * np.conj(x))],
    #               [np.mean(v * np.conj(y)), np.mean(v * np.conj(v)), np.mean(v * np.conj(z)), np.mean(v * np.conj(x))],
    #               [np.mean(z * np.conj(y)), np.mean(z * np.conj(v)), np.mean(z * np.conj(z)), np.mean(z * np.conj(x))],
    #               [np.mean(x * np.conj(y)), np.mean(x * np.conj(v)), np.mean(x * np.conj(z)), np.mean(x * np.conj(x))]])
...
def num_simulation_runs(run_times, snr_val):
    ...
    for i in range(run_times):
        try:
            ...
            ber = calculate_ber(3, calced_rank)  
...
snr_range = [.00000000000000000000000000000000000000000000001, 1, 2, 4, 10, 20, 30, 40]  # Define your range of SNR values # Define your range of SNR values
