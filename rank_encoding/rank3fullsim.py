...
Description:
This script simulates eigenvalue computations of rank-3 signals under varying Signal-to-Noise Ratios (SNR).
...
def compute_eigenvals_rank3(snr_val):
    """
    Computes eigenvalues of a rank-3 signal with added noise at a given SNR.
    """
    phi_1, phi_2, phi_3 = np.pi * np.random.rand(10000), np.pi * np.random.rand(10000), np.pi * np.random.rand(10000)
    E1, E2, E3, Fs, nu = 1, 1, 1, 10000, 10
    t = np.linspace(0, 1 - 1 / Fs, Fs)
    EHa, EVa, EHb, EVb = E1 * np.cos(phi_1), E2 * np.cos(phi_2), E3 * np.cos(phi_3), np.zeros_like(phi_1)
    ...
    G /= np.trace(G)
    return eigh(G, eigvals_only=True), 3
...
def num_simulation_runs(run_times, snr_val):
    ...
    for _ in range(run_times):
        try:
            generated_vector, _ = compute_eigenvals_rank3(snr_val)
