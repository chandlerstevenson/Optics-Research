...
Description:
This script simulates eigenvalue computations of rank-4 signals under varying Signal-to-Noise Ratios (SNR).
...
def compute_eigenvals_rank4(snr_val):
    """
    Computes eigenvalues of a rank-4 signal with added noise at a given SNR.
    """
    phi_1, phi_2, phi_3, phi_4 = np.pi * np.random.rand(10000), np.pi * np.random.rand(10000), np.pi * np.random.rand(10000), np.pi * np.random.rand(10000)
    E1, E2, E3, E4 = np.random.rand(10000), np.random.rand(10000), np.random.rand(10000), np.random.rand(10000)
    Fs, nu = 10000, 10
    t = np.linspace(0, 1 - 1 / Fs, Fs)
    EHa, EVa, EHb, EVb = E1 * np.cos(phi_1), E2 * np.cos(phi_2), E3 * np.cos(phi_3), E4 * np.cos(phi_4)
    ...
    G /= np.trace(G)
    return eigh(G, eigvals_only=True), 4
...
def calculate_ber(original_rank, actual_rank):
    ...
    return ber_map.get(abs(original_rank - actual_rank), abs(original_rank - actual_rank))
